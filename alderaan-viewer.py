...
def display_table_data():
    ...
    Renders index template and displays star table
    """
    global K_id
    global table
    global RESULTS_DIR
...
#@app.route('/get_star_table_dropdown_options')
#def get_star_table_dropdown_options():
...
#@app.route('/get_selected_star_table', methods=['GET'])
...
#@app.route('<koi_id>/review/review_status/', methods=['POST'])
...
def review_status(koi_id):
    ...
    Sends a success message to console(see right_top.html) if review status was written to the table
...
#@app.route('/assign_review_colors_to_star_table/')
#def assign_review_colors_to_star_table():
...
#@app.route('/<koi_id>/plots/generate_plot_detrended_lightcurve')
#def generate_plot_detrended_lightcurve(koi_id):
...
def generate_plot_detrended_light_curve(koi_id):
    ...
    file_path_lc = os.path.join(data_directory, star_id, f"{star_id}_lc_filtered.fits")
    file_path_sc = os.path.join(data_directory, star_id, f"{star_id}_sc_filtered.fits")
...
#@app.route('/<koi_id>/plots/generate_plot_folded_transit')
#def generate_plot_folded_transit(koi_id):
...
def generate_plot_folded_light_curve(koi_id):
    ...
    ### Load data and build overlapping transit mask
    npl = data_load.get_num_planets(file_path_results)
    ...
    for n in range(npl):
        ...
        overlap[n] = np.zeros(len(data_ttv[n].ttime),dtype='bool')
    ...
    ### Get data IDs
    csv_file_path = os.path.join(data_directory, os.path.basename(data_directory) +'.csv')
    ...
    if os.path.isfile(file_path_lc):
        ...
        for n in range(npl):
            if np.sum(~overlap[n]) < 1:
                ...
                ### Initialize maximum likelihood models
                ...
                r_bin = utils.bin_data(t_fold, r_fold, duration/11)[1]
                        
                ### Calculate maximum likelihood model for plotting purposes
                t_mod = np.linspace(-1.5*duration,1.5*duration,50)
                m_mod = batman.TransitModel(theta, t_mod, supersample_factor=1, exp_time=0.02)
                f_mod = m_mod.light_curve(theta)
            
                ### Indexes to thin data to 1000 pts
                npts = 1000
                inds = np.arange(len(t_fold), dtype='int')
                ...
                p_mod = go.Scatter(x=t_mod*24, y=f_mod, mode='lines')
                p_mod.name = "Maximum Likelihood"
                p_mod.legendgroup=f'{n}'
                fig.add_trace(p_mod, row=2*n+1, col=1)
            
                p_resf = go.Scatter(x=t_fold[inds]*24, y=r_fold[inds], mode='markers')
                p_resf.marker.update(symbol='circle', size=5, color='blue')
                fig.add_trace(p_resf, row=2*n+2, col=1)
            
                p_resb = go.Scatter(x=t_bin*24, y=r_bin, mode='markers')
                p_resb.marker.update(symbol='square', size=10, color='orange')
                fig.add_trace(p_resb, row=2*n+2, col=1)
            
                p_mod = go.Scatter(x=t_mod*24, y=np.zeros_like(f_mod), mode='lines')
                fig.add_trace(p_mod, row=2*n+2, col=1)
...
#@app.route('/<koi_id>/plots/generate_plot_individual_transits/<koi_id>_<planet>/<int:line_number>')
#def generate_plot_individual_transits(koi_id, planet, line_number):
...
#@app.route('/<koi_id>/get_planet_options')
#def get_planet_options(koi_id):
...
#@app.route('/<koi_id>/plots/generate_plot_corner/<koi_id>_<planet_num>/<selected_columns>')
